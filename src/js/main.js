// 스크롤 값에 따라 배지영역이 보이거나 보이지 않게 처리
const badgeEl = document.querySelector('header .badges');

// top scroll button
const toTopEl = document.querySelector('#to-top');

// 스크롤 이벤트가 발생하면 매번 값을 반환하는 것이 아닌 콜백 함수로 lodash의 throttle을 사용하여 0.3초마다 실행한 결과 반환하도록 처리
// _.throttle(콜백함수, 시간);
window.addEventListener('scroll', _.throttle(function() {
  console.log(window.scrollY);
  // 스크롤 값이 500 이상이면 보이지 않게 처리
  // 그렇지 않다면 보이게 처리
  if(window.scrollY > 500) {
    // 스크롤 값이 500 이상이 되면 단순히 보이게 처리되는 것이 아닌 부드러운 효과를 적용하고자 함
    // gsap.to(요소, 지속시간, {옵션})
...
// toggle button을 클릭하면 promotion 영역 닫힘 & 열림
...
// 범위 내 숫자를 랜덤 함수(숫자 2자리까지)
// random 함수 인자 최소값, 최대값
function random(min, max) {
  // .toFixed()를 통해 반환된 문자 데이터
  // parseFloat()를 통해 소수점을 가지는 숫자 데이터로 변환
  return parseFloat((Math.random() * (max - min) + min).toFixed(2));
}

// youtube 위 이미지 애니메이션 효과
// gsap.to(요소, 지속시간, {옵션});
function floatingObject(selector, delay, size) {
  gsap.to(selector, random(1.5,2.5), {
    y : size,
    repeat : -1, // 무한 반복, javaScript에서 지원하는 동작
...
floatingObject('img.floating3',1.5,15);

// scrollMagic
const spyEls = document.querySelectorAll('section.scroll-spy');
spyEls.forEach(function(spyEl){
  // Scene() 메서드를 통해 여러 객체들을 변화에 대한 감시 옵션
  // addTo() 라이브러리를 사용하기 위한 옵션들
  new ScrollMagic
  .Scene({
    // 보여질 여부를 검사, 요소 지정
    triggerElement : spyEl,
    // 화면의 높이를 0에서 1이라 가정하면 적용위치 점을 0.8 위치에 적용
    // 기능이 걸려 있는 부분(실행 위치)
    triggerHook : .8
  })
  .setClassToggle(spyEl, 'show') // show class를 높이에 따라 추가/삭제
